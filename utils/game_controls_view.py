...
#         if isEnded:
#             log(f'GAME ENDED FOR HOST {interaction.user} - {interaction.user.id}')
...
class PersistentView(discord.ui.View):
    ...
    async def shuffle_callback(self, interaction: discord.Interaction):
        ...
        if is_host and game_session["chat_id"] == interaction.channel_id:
            await interaction.followup.send("Shuffling...", ephemeral=True)

            current_alpha_ids, current_bravo_ids = await db.operations.get_current_teams(
            ...
            if are_roles_valid:
                current_alpha, current_bravo = await utils.shuffle.split_into_teams(current_role_map)
                new_alpha, new_bravo = await utils.shuffle.shuffle_teams(
                    current_role_map, current_alpha, current_bravo
                )
                new_alpha_ids = [pid for sublist in new_alpha.values() for pid in sublist]
                new_bravo_ids = [pid for sublist in new_bravo.values() for pid in sublist]
                await db.team_change.update_teams(
                    self.bot.db_pool, game_session["game_id"], new_alpha_ids, new_bravo_ids
                )
                ...
                # re-enable if roles invalid
                self.shuffle_button.disabled = False
                msg = await interaction.channel.fetch_message(interaction.message.id)
                await msg.edit(view=self)
                await interaction.followup.send(
                    f"CURRENT ROLES ARE INVALID - {current_role_count}", ephemeral=True
                )
        else:
            # re-enable if not host
            self.shuffle_button.disabled = False
            msg = await interaction.channel.fetch_message(interaction.message.id)
            await msg.edit(view=self)
            await interaction.followup.send(
                "You can only shuffle the 8s session YOU are hosting, not someone else's.",
                ephemeral=True,
            )
    ...
    async def end_callback(self, interaction: discord.Interaction):
        ...
        if isEnded:
            ...
            return await interaction.followup.send(
                embed=BotConfirmationEmbed(description="Game Ended")
            )
        ...
        await interaction.followup.send(
            embed=BotErrorEmbed(description="You are not the host, game was not ended."),
            ephemeral=True,
        )
